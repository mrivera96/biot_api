...
namespace App\biotrackcmdphp;
	...
	class BioTrackCmd
	{
		...
		public function __construct ($type, $ipAddress, $port='')
		{
			switch ($type)
			{
				case 'bioface': case 'bw':
					break;

				default:
					throw new Exception ('Invalid device type specified.');
			}
		...
		{
			$res = shell_exec ($this->path.'biotrackcmd.exe ' . $this->type . ' ' . $this->ipAddress . ($this->port ? ':'.$this->port : '') . ' ' . $cmd);
