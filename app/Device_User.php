<?php

namespace App;

use Illuminate\Database\Eloquent\Model;

class Device_User extends Model
{
    // Nombre de la tabla en SQL server.
    protected $table='Device_User';

    // Eloquent asume que cada tabla tiene una clave primaria con una columna llamada id.
    // Si éste no fuera el caso entonces hay que indicar cuál es nuestra clave primaria en la tabla:
    protected $primaryKey = 'IdUser';

    /**
     * The attributes that are mass assignable.
     * Atributos que se pueden asignar de manera masiva.
     *
     * @var array
     */
    protected $fillable = [
		'IdUser',
      	'IdDevice'
    ];

    /**
     * Indicates if the model should be timestamped.
     * Indicamos si el modelo debe tener la marca de tiempo timestampe|fecha y hora.
     *
     * @var bool
     */
    public $timestamps = false;

    // Definimos a continuación la relación de esta tabla con otras.
    // Ejemplos de relaciones:
    // 1 usuario tiene 1 teléfono   ->hasOne() Relación 1:1
    // 1 teléfono pertenece a 1 usuario   ->belongsTo() Relación 1:1 inversa a hasOne()
    // 1 post tiene muchos comentarios  -> hasMany() Relación 1:N 
    // 1 comentario pertenece a 1 post ->belongsTo() Relación 1:N inversa a hasMany()
    // 1 usuario puede tener muchos roles  ->belongsToMany()
    //  etc..
    // Relación:
    
    public function users()
    {
        // 1 usuario puede tener muchos dispositivos.
        // $this hace referencia al objeto que tengamos en ese momento de Device_User.
        return $this->belongsToMany('App\Device');
    }

    public function devices()
    {
        // 1 dispositivo pertenece a un usuario.
        // $this hace referencia al objeto que tengamos en ese momento de Device_User.
        return $this->belongsTo('App\User');
    }
}
